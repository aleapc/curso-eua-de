<script lang="ts">
  ...
  function importar() {
    ...
    msgOk = !!r;
    msg = r
      ? `Importiert: +${r.ale} von dir, +${r.dea} von ihr/ihm. 🎉`
      : 'Ungültiger Code — prüfe, ob du ihn vollständig kopiert hast (beginnt mit ES1.).';
  }
...
<section class="mt-2">
  ...
  <p class="mt-1 text-carvao/70">
    Survival-Englisch und Kultur für die USA: zuhören, laut aussprechen und nicht als Tourist auffallen.
  </p>
</section>
...
>
  ...
  <span class="min-w-0 flex-1">
    ...
    <span class="block text-xs text-carvao/60"
      >Installiere <b>Google Translate</b> und lade <b>Englisch</b> zur Offline-Nutzung herunter: die
      Kamera liest Speisekarten, Etiketten und Formulare sofort.</span
    >
  </span>
  <span class="shrink-0 text-lg text-carvao/30">↗</span>
</a>
<p class="mt-1 px-1 text-[11px] text-carvao/45">
  💡 Richte ihn VOR dem Abflug ein. Er rettet dich beim Kleingedruckten, aber er bestellt dir nicht
  das Bier und sagt dir nicht, dass dasselbe Getränk drei verschiedene Preise hat.
</p>
...
>
  ...
  <span class="min-w-0 flex-1">
    <span class="block font-bold leading-tight text-carvao">Reisetasche</span>
    <span class="block text-xs text-carvao/60"
      >Währung, Steckertyp und Notrufnummer: funktioniert mit dem Handy im Flugmodus.</span
    >
  </span>
  <span class="shrink-0 text-lg text-carvao/30">→</span>
</a>
  ...
  <section class="mt-7">
          ...
          <div class="space-y-1.5">
            ...
            {#if quizDoEpisodio[ep.id] && ep.partes.some((p) => p.pronta)}
              ...
              >
                ...
                <span class="flex-1 text-sm font-medium">Episoden-Quiz</span>
    ...
    {#if examDoNivel[nivel.nivel] && nivel.episodios.some((e) => e.partes.some((p) => p.pronta))}
      ...
      >
        ...
        <span class="flex-1">
          <span class="block text-sm font-bold">Prüfung {nivel.nome}</span>
          <span class="block text-xs opacity-90">Längeres Audio + Verständnisprüfung</span>
        </span>
...
<section class="mt-8">
  <div class="card p-4">
    <h2 class="mb-1 font-bold">👯 Synchronisiere mit deinem Reisebegleiter</h2>
    <p class="mb-3 text-sm text-carvao/60">
      Erzeuge einen Code und schick ihn, um zusammenzuführen, was ihr beide geschafft habt: ohne
      Konto, ohne Server.
    </p>
    <div class="flex flex-wrap gap-2">
      <button type="button" class="btn-primary" onclick={gerar}>Meinen Code holen</button>
      {#if codigo}
    ...
    <div class="mt-4 border-t border-black/5 pt-3">
      <p class="mb-2 text-sm text-carvao/60">Hast du einen Code? Füg ihn hier ein:</p>
      <textarea
        bind:value={cola}
        placeholder="ES1...."
